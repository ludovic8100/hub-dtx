...
export default function DashboardLode() {
  ...
  useEffect(() => {
    async function load() {
      const { data: socData } = await supabase.from('societes').select('id').eq('code', 'LODE').single()
    ...
    <Layout currentPage="Tableau de bord">
      <div style={{ fontFamily:"'Source Sans Pro', sans-serif", maxWidth:1200 }}>
        ...
        <div style={{ marginBottom:24 }}>
          <h1 style={{ fontSize:22, fontWeight:800, color:'#7c2d12', margin:'0 0 4px' }}>Bonjour {firstName} 👋</h1>
          <p style={{ fontSize:14, color:'#64748b', margin:0 }}>{'LODE SRL'} — aperçu de l'activité</p>
        </div>
        ...
        <div style={{ marginBottom:24 }}>
          <BlocComptes societeCode="LODE" color="#ea580c" />
        </div>
