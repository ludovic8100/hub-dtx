  ...
  useEffect(() => {
    async function load() {
      const [{ data: c }, { data: t }] = await Promise.all([
        supabase.from('comptes_bancaires').select('nom, solde_actuel, devise, iban').eq('societe_id', 'LODE'),
    ...
    <Layout currentPage="Tableau de bord">
      <div style={{ fontFamily: "'Source Sans Pro', sans-serif", maxWidth: '1200px' }}>
        <div style={{ marginBottom: '28px' }}>
          <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#7c2d12', margin: '0 0 4px' }}>Bonjour {firstName} 👋</h1>
          <p style={{ fontSize: '14px', color: '#64748b', margin: 0 }}>LODE SRL — aperçu de l'activité</p>
        </div>

        <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ padding: '14px 20px', background: '#ea580c', color: '#fff', fontWeight: '700', fontSize: '14px' }}>Comptes bancaires</div>
          {comptes.length === 0 ? (
            ...
            return (
              <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 20px', borderBottom: i < taches.length - 1 ? '1px solid #f8fafc' : 'none', background: retard ? '#fff5f5' : '#fff' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '11px', fontWeight: '600', padding: '3px 8px', borderRadius: '4px', background: retard ? '#fee2e2' : '#fff7ed', color: retard ? '#dc2626' : '#ea580c' }}>
