  ...
  return (
    <div style={{ background:'#fff', borderRadius:12, padding:20, border:'1px solid #e2e8f0', borderTop:`3px solid ${color}` }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          <div style={{ fontSize:11, color:'#94a3b8', fontWeight:700, textTransform:'uppercase', letterSpacing:'.05em', marginBottom:8 }}>{}</div>
          <div style={{ fontSize:28, fontWeight:800, color:'#0f172a', lineHeight:1 }}>{}</div>
          {sub && <div style={{ fontSize:12, color:'#94a3b8', marginTop:4 }}>{}</div>}
        </div>
...
export default function DashboardHexagroup() {
  ...
  useEffect(() => {
    async function load() {
      const { data: socData } = await supabase.from('societes').select('id').eq('code', 'HEXAGROUP').single()
    ...
    <Layout currentPage="Tableau de bord">
      <div style={{ fontFamily:"'Source Sans Pro', sans-serif", maxWidth:1200 }}>
        ...
        <div style={{ marginBottom:24 }}>
          <h1 style={{ fontSize:22, fontWeight:800, color:'#7f1d1d', margin:'0 0 4px' }}>Bonjour {firstName} 👋</h1>
          <p style={{ fontSize:14, color:'#64748b', margin:0 }}>{'Hexagroup ASBL'} — aperçu de l'activité</p>
        </div>
        ...
        <div style={{ marginBottom:24 }}>
          <BlocComptes societeCode="HEXAGROUP" color="#dc2626" />
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:12, border:'1px solid #e2e8f0', overflow:'hidden' }}>
          ...
          {loading ? <div style={{ padding:30, textAlign:'center', color:'#94a3b8' }}>Chargement…</div>
          : taches.length === 0 ? <div style={{ padding:30, textAlign:'center', color:'#16a34a', fontSize:13 }}>✓ Aucune tâche en cours</div>
          : (
