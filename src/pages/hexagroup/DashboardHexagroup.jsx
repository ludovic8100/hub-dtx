    ...
    async function load() {
      ...
      const [{ data: tsk }, { data: txs }] = await Promise.all([
        supabase.from('taches').select('id,titre,gestionnaire,echeance,statut')
          .in('statut', ['todo', 'en_cours', 'en_attente', 'retard']).eq('entite', 'hexagroup').order('echeance', { ascending: true }).limit(10),
    ...
    <Layout currentPage="Tableau de bord">
      <div style={{ fontFamily: "'Source Sans Pro', sans-serif", width: '100%' }}>
        <StatBanner
          ...
          subtitle="Hexagroup ASBL — aperçu de l'activité"
          stats={[
        ...
        <div style={{ marginBottom: 22 }}>
          {isAdmin && <BlocComptes societeCode="HEXAGROUP" color={E.color} />}
        </div>
