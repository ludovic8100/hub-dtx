...
import { DTX, TVA_TAUX, CGV, DELAI_PAIEMENT_JOURS } from '../../lib/dtxConfig'
...
function Editeur({ type, doc, onClose, onSaved }) {
  ...
  const table = isDevis ? 'dtx_devis' : 'dtx_factures'
  const tableLignes = isDevis ? 'dtx_devis_lignes' : 'dtx_factures_lignes'
  ...
  useEffect(() => {
    supabase.from('dtx_clients').select('*').eq('actif', true).order('denomination', { nullsFirst: false })
  ...
  const save = async () => {
    ...
    try {
      ...
      if (docId) {
        ...
        const { data: num } = await supabase.rpc('next_dtx_numero', { p_type: type })
        payload.numero = num
...
async function exportPDF(type, doc, lignes) {
  ...
  // ---- Logo DTX (en-tête, à gauche) ----
  if (DTX.logo_url) {
    const logo = await loadImageDataURL(DTX.logo_url)
  ...
  d.text(DTX.raison_sociale, 16, yDeA + 7)
  d.setFontSize(9); d.setTextColor(...DARK); d.setFont(undefined, 'normal')
  d.text([DTX.adresse, `${DTX.cp} ${DTX.ville}`, DTX.pays || 'Belgique', `TVA ${DTX.tva}`], 16, yDeA + 13)
  ...
  d.text(`${L.paiement} : ${DTX.iban}  (${DTX.bic} – ${DTX.banque})`, 16, y)
...
async function exportExcel(type, doc, lignes) {
  ...
  const L = I18N[doc.langue] || I18N.fr
  const rows = [
    [DTX.raison_sociale, '', '', isDevis ? L.devis : L.facture],
    [DTX.adresse, '', '', doc.numero],
    [`${DTX.cp} ${DTX.ville}`, '', '', isDevis ? `${L.validite} : ${fmtDate(doc.date_validite)}` : `${L.echeance} : ${fmtDate(doc.date_echeance)}`],
    [`TVA ${DTX.tva}`, '', '', ''],
...
function SuiviModal({ doc, color, onClose, onChanged }) {
  ...
  const charge = () => supabase.from('dtx_devis_events').select('*').eq('devis_id', doc.id)
    .order('created_at', { ascending: true }).then(({ data }) => setEvents(data || []))
  useEffect(() => {
    charge()
    supabase.from('dtx_devis_lignes').select('*').eq('devis_id', doc.id).order('position', { ascending: true })
      .then(({ data }) => setLignes(data || []))
  }, [doc.id])

  async function logEvent(type, detail) {
    await supabase.from('dtx_devis_events').insert({ devis_id: doc.id, type, detail })
  }
  async function marquerEnvoye(viaEmail) {
    ...
    await supabase.from('dtx_devis').update({
  ...
  async function changeStatut(s) {
    ...
    await supabase.from('dtx_devis').update(patch).eq('id', doc.id)
    ...
    try {
      ...
      const r = await fetch('/api/devis-send', {
        ...
        body: JSON.stringify({
          ...
          base: window.location.origin, entite: DTX.entite,
        }),
      })
        ...
        <div style={{ flex: 1, background: '#f1f5f9', overflow: 'auto', padding: mob ? 14 : 28, order: mob ? 2 : 1 }}>
          <div style={{ background: '#fff', maxWidth: 620, margin: '0 auto', borderRadius: 10, boxShadow: '0 2px 12px rgba(0,0,0,.08)', padding: mob ? '22px 18px' : '30px 34px', fontSize: 12, color: '#1e293b' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 22 }}>
              {DTX.logo_url ? <img src={DTX.logo_url} alt="DTX" style={{ height: 46 }} /> : <div style={{ fontWeight: 800, fontSize: 18, color }}>{DTX.raison_sociale}</div>}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 20, marginBottom: 20 }}>
              <div>
                ...
                <div style={{ fontWeight: 800 }}>{DTX.raison_sociale}</div>
                <div style={{ color: '#475569' }}>{DTX.adresse}<br />{DTX.cp} {DTX.ville}<br />{DTX.pays}</div>
                <div style={{ color: '#475569', marginTop: 4 }}>TVA : {DTX.tva}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 22, paddingTop: 12, borderTop: '1px solid #e2e8f0', color: '#64748b', fontSize: 11 }}>
              <div>{DTX.iban && <>IBAN : {DTX.iban}<br /></>}{DTX.bic && <>BIC : {DTX.bic}</>}</div>
              <div style={{ textAlign: 'right' }}>{DTX.email}{DTX.telephone && <><br />{DTX.telephone}</>}</div>
            </div>
          </div>
        </div>
...
export default function DtxDevisFactures() {
  ...
  const load = async () => {
    ...
    const [d, fa] = await Promise.all([
      supabase.from('dtx_devis').select('*').order('created_at', { ascending: false }),
      supabase.from('dtx_factures').select('*').order('created_at', { ascending: false }),
    ])
  ...
  const getLignes = async (type, id) => {
    const t = type === 'devis' ? 'dtx_devis_lignes' : 'dtx_factures_lignes'
  ...
  const envoyerPeppol = async (doc) => {
    ...
    try {
      ...
      if (data && data.ok) {
        await supabase.from('dtx_factures').update({ statut: 'envoyée' }).eq('id', doc.id)
      ...
      alert(`Échec de l'envoi Peppol : ${e.message}\n\nVérifie que le workflow n8n « DTX - Peppol Send » est actif et que la clé API Billit est renseignée.`)
    } finally { setBusy(null) }
  }
  ...
  const convertir = async (devisDoc) => {
    ...
    const { data: num } = await supabase.rpc('next_dtx_numero', { p_type: 'facture' })
    const { data: fact, error } = await supabase.from('dtx_factures').insert({
    ...
    if (lignes.length) {
      await supabase.from('dtx_factures_lignes').insert(lignes.map((l, i) => ({
  ...
  const supprimer = async (type, id) => {
    ...
    await supabase.from(type === 'devis' ? 'dtx_devis' : 'dtx_factures').delete().eq('id', id)
    load()
  }
  ...
  return (
    <Layout currentPage="Devis & Factures">
      <div style={{ fontFamily: "'Source Sans Pro', sans-serif", width: '100%' }}>
        <StatBanner
          color={C} colorDark={C_DARK} logoUrl={DTX.logo_url}
          title={tab === 'devis' ? 'Devis' : 'Factures'}
          subtitle={`DTX SRL — ${DTX.activite}`}
          stats={stats}
